#!/bin/bash
# Grid-b serial runner: trains 6 b values for seed 123
# Each run: 200M tokens, Qwen3-0.6B, weighted pretraining
# Input: data/scored_data.jsonl (371K docs with q_score)
# Output: output/models/grid_b{B}_seed123/ for each b
# Expected runtime: ~12h (6 runs x ~2h each)
...
for b in 0.5 1.0 1.5 2.0 3.0 5.0; do
    echo "=========================================="
    echo "Starting grid-b=${b} seed=123 at $(date)"
    echo "=========================================="
    ...
    python training/pretrain_weighted.py \
        ...
        --output_dir output/models/grid_b${b_dir}_seed123 \
    ...
    echo "Completed grid-b=${b} seed=123 at $(date)"
    echo ""
done

echo "All grid-b runs for seed 123 complete at $(date)"
