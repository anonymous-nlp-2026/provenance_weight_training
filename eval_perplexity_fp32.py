...
def eval_perplexity(model, tokenizer, texts, max_length=2048, batch_size=8):
    ...
    for doc_idx, text in enumerate(texts):
        ...
        if (doc_idx + 1) % 500 == 0:
            ...
            print(f"  [{doc_idx+1}/{len(texts)}] tokens={total_tokens}, loss={interim_loss:.6f}")
...
def main():
    ...
    for ckpt_path in args.checkpoints:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ckpt_path, torch_dtype=torch.float32, trust_remote_code=True
        ).cuda().eval()
        ...
        print(f"  Loss: {result['loss']:.6f}  Perplexity: {result['perplexity']:.6f}  "
    ...
    for name, r in results.items():
        print(f"{name:<30} {r['loss']:>10.6f} {r['perplexity']:>14.6f} {r['num_tokens']:>10}")
    print("="*70)
