...
def find_b_np(q, tau=0.7, n_min=2, tau_delta=0.3, tau_min=0.5, b_max=20.0, res=200):
...
print('=== Part 1: Empirical alpha_eff variance ===', flush=True)
def load_sm(p):
    with open(p) as f:
...
for lab, ps in [('B=4', b4p), ('B=16', b16p)]:
    ...
    for p in ps:
        ...
        print(f'  {nm}: n={len(m)} a_mean={np.mean(a):.4f} a_var={np.var(a):.6f}')
    arr = np.array(aa)
...
print(f'mean_q var ratio: {emp["B=4"]["mq_var"]/emp["B=16"]["mq_var"]:.2f}', flush=True)

print('')
print('=== Part 2: Simulation ===', flush=True)
data_q, data_r = [], []
...
print(f'N={len(qa)}, real_frac={ra.mean():.4f}', flush=True)

np.random.seed(42)
NS = 2000
tau_val, nm_val = 0.7, 2
sr = {}

for B in [4, 16]:
    tf, aad, ab0, abn, bs_list = [], [], [], [], []
    for i in range(NS):
        idx = np.random.choice(len(qa), size=B, replace=False)
        bq, br = qa[idx], ra[idx]
        bst, _ = find_b_np(bq, tau=tau_val, n_min=nm_val)
    ...
    print(f'  b*: mean={bs_arr.mean():.2f} std={bs_arr.std():.2f} b0={float((bs_arr==0).mean()):.3f}', flush=True)
    sr[f'B{B}'] = {'r2_soft': r2b0, 'r2_bin': r2bn, 'r2_adp': r2ad, 'gap': gap,
                   ...
                   'mean_b': float(bs_arr.mean()), 'frac_b0': float((bs_arr==0).mean())}

print('')
print('=== Part 3: Detector ===', flush=True)
...
for lo, hi in [(0.3,0.7),(0.4,0.6),(0.45,0.55)]:
    ...
    rfrac = float((mk & (ra==1)).sum() / n) if n>0 else 0
    print(f'  q[{lo},{hi}]: {n}({n/len(qa)*100:.1f}%) acc={ac:.3f} real_frac={rfrac:.3f}')

print('')
print('=== Part 4: Theoretical batch noise ===', flush=True)
ps = 1-ra.mean()
from scipy.stats import binom as bn
for B in [4,8,16,32]:
    bv = ps*(1-ps)/B
    msg = f'  B={B:2d}: Var={bv:.6f} Std={np.sqrt(bv):.4f}'
    if B <= 16:
        msg += f' P(all_s)={bn.pmf(B,B,ps):.6f} P(all_r)={bn.pmf(0,B,ps):.6f}'
    print(msg)

out = {'empirical': emp, 'var_ratio': vr, 'simulation': sr,
       'detector': {'acc': acc, 'fp': fp, 'fn': fn,
                    'bnd_04_06': int(((qa>=0.4)&(qa<=0.6)).sum()),
                    'bnd_pct': float(((qa>=0.4)&(qa<=0.6)).mean())},
